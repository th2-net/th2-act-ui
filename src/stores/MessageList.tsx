...
export const Indicators = (props: {indicator: string}) => (
	<div>
		<button className={props.indicator}></button>
	</div>
);
...
export const MessageList = (props: {indicators: string[]; items: string[]; editMessageMode: boolean;
	...
	const {
		parsedMessagesHistory, clearParsedMessages, setEditorCode, deleteIndicator, addParsedMessage,
		setDictionaryName, setSelectedMessageType, setSelectedSession, setSendButtonTitle, setSendButtonIcon,
		setEditMessageMode, setEditedMessageIndex, setEditedMessageSendDelay,
	} = useStore();
	...
	const selectMessage = (index: number) => {
		setSelectedSession(JSON.parse(parsedMessagesHistory[index]).session);
		setDictionaryName(JSON.parse(parsedMessagesHistory[index]).dictionary);
		setSelectedMessageType(JSON.parse(parsedMessagesHistory[index]).messageType);
		setEditorCode('{}'); // JSON.parse(parsedMessagesHistory[index]).messageType
		setSendButtonTitle('Save');
		setSendButtonIcon('');
	...
	return (
		<ul>
			...
			<li key={props.items.length + 1}>
				<div className={props.editMessageMode ? 'normalNewMessage' : 'hiddenNewMessage'}
						...
						<div className="cardItems">
							<div>
								<button className="deleteButton" onClick={() => {
									if (props.editMessageMode) deleteMessage(index);
								}}>x</button>
							</div>
							<Indicators indicator={props.indicators[index]} />
							<div className={props.editMessageMode
