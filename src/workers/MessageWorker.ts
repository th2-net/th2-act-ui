...
import { action as mobxAction, makeObservable, observable, reaction } from 'mobx';
...
export default class MessageWorker {
	constructor() {
		makeObservable(this, {
			...
			subscribeOnMessage: mobxAction,
			unsubscribeFromMessage: mobxAction,
		});

		reaction(
			() => this.subscribers.size,
			size =>
				(size ? window.addEventListener : window.removeEventListener)('message', this.handleIncomingMessage),
		);
	}

	subscribers: Map<IncomingMessageActions, Function[]> = new Map();
	...
	public unsubscribeFromMessage = (
		...
		this.subscribers.set(
			action,
			subGroup.filter(cb => cb !== callback),
		);
	};
